...
- Modelos YOLO definidos no `.env` (`YOLO_POSE_MODEL` e `YOLO_OBJ_MODEL`) no diretório raiz
...
Certifique-se de que os arquivos configurados em `.env` (`YOLO_POSE_MODEL` e `YOLO_OBJ_MODEL`) estão no diretório raiz:
